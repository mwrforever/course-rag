 ...
 * 路由 /teachers 由守卫限定仅超管可入（权限契约不变，本页不含权限逻辑）。
 * 权限矩阵：自身行禁用/启用入口隐藏（防自锁，后端 A7 同时拒绝）。
 */
import { computed, reactive, ref } from 'vue'
import { useMutation, useQuery, useQueryClient } from '@tanstack/vue-query'
import { z } from 'zod'
import { PhChalkboardTeacher, PhSpinnerGap, PhUserPlus } from '@phosphor-icons/vue'
...
<template>
  <div>
    <!-- 页头：主标题 + 副题 + 右侧「添加教师」主操作（设计稿 page-head 形态） -->
    <PageHead v-reveal title="教师管理" subtitle="管理教师账号、状态与密码">
      <template #actions>
    ...
    >
      <EmptyState title="还没有教师" description="创建教师账号后即可由教师登录使用课程助手">
        <template #icon>
          <PhChalkboardTeacher class="h-6 w-6" aria-hidden="true" />
        </template>
        <template #action>
    ...
    <template v-else>
      <div v-reveal class="overflow-hidden rounded-2xl border border-border bg-surface shadow-xs">
        <DataTable data-testid="teacher-table" label="教师列表">
          <template #header>
            <tr>
          ...
          <tr v-for="u in teachers" :key="u.id" :data-testid="`row-${u.id}`">
    ...
    <!-- 添加教师 Dialog（角色固定 TEACHER，无角色选择器；表单 Dialog 保留内联实现承载 zod 校验） -->
    <div
      v-if="addOpen"
    ...
    >
      <div
        ...
        @click.stop
      >
        <h2 class="text-lg font-bold text-text">添加教师</h2>
    ...
    <div v-if="statusTarget" data-testid="status-dialog" class="contents">
      <ConfirmDialog
        :open="true"
        :title="statusNext === 'DISABLED' ? '禁用教师' : '启用教师'"
        :description="
          statusNext === 'DISABLED'
    ...
    <!-- 删除教师二次确认（ConfirmDialog：confirm-user-del 选择器契约同上） -->
