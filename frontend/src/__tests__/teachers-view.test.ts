...
async function mountAt(path = '/teachers') {
  ...
  const wrapper = mount(TeachersView, { global: { plugins: [pinia, router] } })
...
describe('教师管理（/teachers，仅超管）', () => {
  it('加载教师分页列表：role=TEACHER 过滤 + 分页器', async () => {
    apiMock.userApi.list.mockResolvedValue(pageOf([student('s-1')], '11'))
    const { wrapper } = await mountAt()
    await flushPromises()

    expect(apiMock.userApi.list).toHaveBeenCalledWith(
      expect.objectContaining({ role: 'TEACHER', page: 1, size: 10 }),
    )
    expect(wrapper.find('[data-testid="teacher-table"]').exists()).toBe(true)
    expect(wrapper.find('[data-testid="row-s-1"]').text()).toContain('教师s-1')
  ...
  it('空列表：语义空态 + 添加教师入口', async () => {
    apiMock.userApi.list.mockResolvedValue(pageOf([]))
    const { wrapper } = await mountAt()
    await flushPromises()
    expect(wrapper.text()).toContain('还没有教师')
    wrapper.unmount()
  })

  it('添加教师：zod 校验失败就地报错；合法提交 create 固定 TEACHER 角色', async () => {
    apiMock.userApi.list.mockResolvedValue(pageOf([]))
    ...
    await wrapper.find('[data-testid="add-username"]').setValue('tealiu')
    ...
    expect(showToast).toHaveBeenCalledWith('教师账号已创建', 'success')
    wrapper.unmount()
  })
  ...
  it('禁用/启用二次确认与删除二次确认', async () => {
    ...
    expect(showToast).toHaveBeenCalledWith('已禁用该教师', 'success')
    ...
    expect(showToast).toHaveBeenCalledWith('教师已删除', 'success')
    wrapper.unmount()
  })
  ...
  it('加载失败：横幅 + 重试恢复', async () => {
    ...
    await wrapper.find('[data-testid="retry-teachers"]').trigger('click')
    await flushPromises()
    expect(wrapper.find('[data-testid="teacher-table"]').exists()).toBe(true)
    wrapper.unmount()
  })
