...
/**
 * 附件 chips 与前置校验（设计 §1.5.4 附件规范 + G11 白名单）
 *
 ...
 * 或文档图标 + 字节大小）/ 失败（可移除）。移除回调携带本地 id。
 * 线程安全：纯展示组件 + 纯函数校验，无共享可变状态。
 */
import { FileText, Image, X } from "@phosphor-icons/react";
...
/**
 * 附件 chips 行（上传中进度环 / 图片缩略图 / 文档图标 + 移除）
 *
      ...
      {items.map((item) => (
        ...
        >
          ...
          <div className="min-w-0">
            <p className="max-w-40 truncate text-xs font-medium text-text">{item.file.name}</p>
