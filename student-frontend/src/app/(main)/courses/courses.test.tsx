/**
 * 课程列表页测试（Task 9 TDD 先行用例）
 *
 * 设计 §1.5.2：本地 category 筛选（Chip 从 J1 数据聚合，选中 teal-soft）+ 关键词
 * 即时过滤 + 排序（默认评分降序 / 按名称）+ 本地分页（每页 12）+ URL query 浅路由
 * 同步（?category=&q=）。四态：Loading 骨架 / Empty / Error 横幅+重试 / 正常态。
 */
...
describe("课程列表页筛选/搜索/排序", () => {
  ...
  it("category Chip 组从 J1 数据聚合（去重），点击后仅显示该类课程并浅路由同步", async () => {
    apiMock.getMyCourses.mockResolvedValue([
    ...
    // 点击「数学」：只留数学课程，chips 选中态 teal-soft，URL 同步
  ...
  it("关键词即时过滤（匹配标题），与 category 组合生效并同步 URL", async () => {
    apiMock.getMyCourses.mockResolvedValue([
...
describe("课程列表页 URL query 驱动与本地分页", () => {
  ...
  it("分页：13 门课每页 12，翻页后展示末条并禁用边界按钮", async () => {
    ...
    apiMock.getMyCourses.mockResolvedValue(courses);
    renderList();
