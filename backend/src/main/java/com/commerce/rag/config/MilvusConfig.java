 ...
 * 连接参数经 {@link MilvusProperties}（milvus.host / milvus.port）强类型注入
...
public class MilvusConfig {
     ...
     *
     * @param milvusProperties Milvus 全量配置（host/port 连接参数 + rpcDeadlineMs 调优参数，非空，Spring 注入）
     * @return MilvusClientV2 实例
     */
    @Bean
    public MilvusClientV2 milvusClientV2(MilvusProperties milvusProperties) {
        String uri = "http://" + milvusProperties.host() + ":" + milvusProperties.port();
