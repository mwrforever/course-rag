...
public class AttachmentImageProcessor {
    ...
    public List<ImageCaptionResult> processImages(List<byte[]> images, List<String> names) {
        ...
        for (int i = 0; i < images.size(); i++) {
            ...
            try {
                futures.add(CompletableFuture.supplyAsync(() -> captionOne(bytes, name, position + 1), attachmentPool));
     ...
     */
    private ImageCaptionResult captionOne(byte[] bytes, String name, int index) {
        try {
