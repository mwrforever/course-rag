 ...
 *
 * <p>权限控制：教师只能操作自己创建的课程下的排期（通过 ICourseService.checkOwnership 校验）。
 *
 * @author commerce-rag
 */
@Service
...
public class CourseScheduleServiceImpl extends ServiceImpl<CourseScheduleMapper, CourseSchedule>
    ...
    private static final Logger log = LoggerFactory.getLogger(ICourseScheduleService.class);
    ...
    public void update(Long id, UpdateScheduleRequest request, Long currentUserId, boolean isAdmin) {
        CourseSchedule schedule = scheduleMapper.selectById(id);
        if (schedule == null) {
            throw new BizException(ErrorCode.NOT_FOUND, "排期不存在: " + id);
        }
        courseService.checkOwnership(schedule.getCourseId(), currentUserId, isAdmin);
